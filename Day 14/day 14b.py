...
def find_partition(pos, grid):
    found = []
    targets = [pos]
    while len(targets) > 0:
        cur_target = targets[0]
        found.append(cur_target)
        del targets[0]
        for delta in (-1, 1):
            # check x + delta, y
            check_target_x = cur_target[0] + delta
            if 0 <= check_target_x < 128:
                cur_pos = (check_target_x, cur_target[1])
                if grid[cur_target[1]][check_target_x] == '1' and cur_pos not in found and cur_pos not in targets:
                    targets.append(cur_pos)
            # check x , y + delta
            check_target_y = cur_target[1] + delta
            if 0 <= check_target_y < 128:
                cur_pos = (cur_target[0], check_target_y)
                if grid[check_target_y][cur_target[0]] == '1' and cur_pos not in found and cur_pos not in targets:
                    targets.append(cur_pos)
    return found
...
for i in range(128):
    puzzle_string = puzzle_input + '-' + str(i)
...
for y in range(128):
    for x in range(128):
        pos = (x, y)
        if output_lst[y][x] == '1' and pos not in positions_found:
            answer += 1
            additional_pos = find_partition(pos, output_lst)
            # print(additional_pos)
            # breakpoint()
            positions_found += additional_pos
